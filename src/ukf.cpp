...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  if (!is_initialized_) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      Initialize state.
      */
      if (abs(meas_package.raw_measurements_[0]) < 0.000001 or abs(meas_package.raw_measurements_[1]) < 0.000001) {
        x_ << 0.000001, 0.000001, 0, 0, 0;
      } else {
  ...
  if (delta_t > 0.0001) {
    try {
      ...
      //Prediction(delta_t);
      std::cout << "caught numerical instability" << std::endl;
      while (delta_t > 0.14 ) {
  ...
  */
   ...
   *  Update
   ****************************************************************************/
   //std::cout << "start update measurement" << std::endl;
   if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
...
void UKF::GenerateAugmentedSigmaPoints(const VectorXd &x, const MatrixXd &P, MatrixXd &Xsig_aug) {
  /***********************************************
   begin calculating augmented sigma points
   ***********************************************/
...
void UKF::SigmaPointPrediction(double delta_t, const MatrixXd &Xsig_aug, MatrixXd &Xsig_pred) {
  ...
  /***************************************************
   End calculating predicted sigma points
   ****************************************************/
...
void UKF::PredictMeanAndCovariance(const MatrixXd &Xsig_pred, VectorXd &x, MatrixXd &P) {
  /***************************************************
   Start calculating Mean and covariance
   ****************************************************/
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
    ...
    x_diff(3) = normalizeRadianPi(x_diff(3));

    P = P + weights_(i) * x_diff * x_diff.transpose() ;
  }
  /***************************************************
   End calculating Mean and covariance
   ****************************************************/
}
...
void UKF::PredictRadarMeasurement(const MatrixXd &Xsig_pred, MatrixXd &Zsig, VectorXd &z_pred, MatrixXd &S) {
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    double v1 = cos(yaw)*v;
    double v2 = sin(yaw)*v;
    if (abs(p_x) < 0.000001 or abs(p_y) < 0.000001 ) {
      Zsig(0,i) = 0;                        //r
      Zsig(1,i) = 0;                        //phi
      Zsig(2,i) = 0;                        //r_dot
...
void UKF::UpdateState(const VectorXd &z, const VectorXd &z_pred, const MatrixXd &S,
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
    ...
    //residual
    VectorXd z_diff = Zsig.col(i) - z_pred;
    z_diff(1) = normalizeRadianPi(z_diff(1));
